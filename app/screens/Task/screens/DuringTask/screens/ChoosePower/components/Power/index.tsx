...
const Power = ({ title, status, image, description, people }: Props) => {
    ...
    const [peopleList, setPeopleList] = useState(people ? people : [] as string[])
    ...
    return (
        <Pressable style={getStyles(theme).container} onPress={handleOnPress}>
            ...
            <View style={getStyles(theme).textContainer}>
                ...
                {
                    peopleList.length > 0 ? <View style={getStyles(theme).selectionBlur}>
                        {
                            peopleList.map((person, index) => (
                                <Text style={getStyles(theme).people} key={index}>{person}</Text>
                            ))
